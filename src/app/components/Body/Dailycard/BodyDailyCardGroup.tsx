...
import { GetUsedCard } from './GetUsedCard';
export const BodyDailyCardGroup = () => {
  const [FreeSpreadCard, setFreeSpreadCard]: any = React.useState(false);
  const [changetimesetView, setChangetimesetView] = useState(false)
  ...
  const spreadCardFirst: any = () => {
    ...
    dispatch(DailyCardRDucer.GetIndexCard(0));
  };
  ...
  return (
    <Flex
      ...
      style={{ width: '100%', height: '100vh' }}
    >
      {selector.Filter ? <GetUsedCard /> : <Oldpage />}
    </Flex>
  );
};
